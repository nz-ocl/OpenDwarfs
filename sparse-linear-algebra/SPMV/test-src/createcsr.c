...
int main(int argc, char** argv)
{
  ...
  if(do_save)
  {
	if(!file_path)
	{
		...
		snprintf(file_path,(sizeof(char)*CSR_NAME_MAX_LENGTH),"../test/sparse-linear-algebra/SPMV/csrmatrix_N%lu_D%lu_S%02d_%d-%d-%d-%d-%d",N,density,normal_stddev_rounded,(tm.tm_year+1900),tm.tm_mon+1,tm.tm_mday,tm.tm_hour,tm.tm_min);
		free_file=1;
	}
